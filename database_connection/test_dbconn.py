...
class TestDBConn(unittest.TestCase):
    ...
    DATA_FILENAME = 'test_data.sql'
    REMOVE_DATA_FILENAME = 'test_remove_data.sql'
    ...
    # TODO(Alex): add TUnit and question unittests once format is finalized
    ...
    # TODO(Alex): add vote unittests when TUnit format is finalized

    def test_select_articles_location_positive(self):
        exp_articles = [{'article_id': 0, 'title': 'a'}, {'article_id': 1, 'title': 'b'}]
        location = (30.1, 30.1)
    ...
    def test_select_articles_location_negative(self):
        exp_articles = [{'article_id': 2, 'title': 'c'}]
        location = (-30.1, 30.1)
        act_articles = DBConn(TestDBConn.DB_FILENAME).select_articles_location(*location)
        self.assertEqual(exp_articles, act_articles)
